...
class Board {
    ...
    constructor(rawString: string) {
        const rows: string[][] = rawString.split('\r\n').map(line => line.split(' ').filter(val => val.length > 0))
        const cols: string[][] = [[], [], [], [], []]
    ...
    public evaluateBoard(calls: string[]): BoardResult | null {
        ...
        for (let call of calls) {
            ...
            if (this.rowsAndCols.some((set: string[]) => set.every(val => calledNumbers.includes(val)))) {
                return {
...
export class Day04 extends Day {
    ...
    protected solvePartOne(input: string): number {
        ...
        const boardResults: BoardResult[] = boards.map(board => board.evaluateBoard(calls)).filter(res => res != null) as BoardResult[];

        const bestBoard: BoardResult = boardResults.reduce((prevBest, curr) => !!prevBest && prevBest.roundsToBingo < curr.roundsToBingo ? prevBest : curr, {roundsToBingo: 100000, score: 0})
    ...
    protected solvePartTwo(input: string): number {
        ...
        const worstBoard: BoardResult = boardResults.reduce((prevWorst, curr) => !!prevWorst && prevWorst.roundsToBingo > curr.roundsToBingo ? prevWorst : curr, {roundsToBingo: 0, score: 0})
