...
class SegmentMapping {
  ...
  constructor(allCombinations: string[]) {
    ...
    allCombinations.forEach((element) => {
      ...
      element
        .split("")
        .forEach(
          (char) => (letterCounts[char] = (letterCounts[char] || 0) + 1)
        );
    });

    this.segmentMapping = {
      a: this.getKeysWithValue(letterCounts, 8).filter(
        (key) => !segmentsInOne.includes(key)
      )[0],
      b: this.getKeysWithValue(letterCounts, 6)[0],
      c: this.getKeysWithValue(letterCounts, 8).filter((key) =>
        segmentsInOne.includes(key)
      )[0],
      d: this.getKeysWithValue(letterCounts, 7).filter((key) =>
        segmentsInFour.includes(key)
      )[0],
      e: this.getKeysWithValue(letterCounts, 4)[0],
      f: this.getKeysWithValue(letterCounts, 9)[0],
      g: this.getKeysWithValue(letterCounts, 7).filter(
        (key) => !segmentsInFour.includes(key)
      )[0],
    };
  }

  private getKeysWithValue(
  ...
  public getNumberFromSegments(segments: string): number {
    return Object.keys(RequiredSegments)
      ...
      .filter((value: number) => {
        const mappedRequiredSegments: string[] = RequiredSegments[value].map(
          (val) => this.segmentMapping[val]
        );

        return (
          mappedRequiredSegments.length == segments.length &&
          mappedRequiredSegments.every((req) => segments.includes(req))
        );
      })[0];
  }
}
...
export class Day08 extends Day {
  public solvePartOne(input: string): number {
    const outputs: string[] = splitLines(input).flatMap((row) =>
      row.split(" | ")[1].split(" ")
    );

    return outputs.filter((value) => [2, 3, 4, 7].includes(value.length))
      .length;
  }
  ...
  public solvePartTwo(input: string): number {
    ...
    lines.forEach((line) => {
      ...
      const segmentMapping: SegmentMapping = new SegmentMapping(
        lineParts[0].split(" ")
      );

      const numbers: number[] = lineParts[1]
        .split(" ")
        .map((val) => segmentMapping.getNumberFromSegments(val));
