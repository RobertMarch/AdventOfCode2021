...
class Line {
    ...
    public getAllPoints(): number[][] {
        ...
        for (let step = 0; step <= numberOfPoints; step++) {
            const x: number = (this.x1 * step/numberOfPoints) + (this.x2 * (numberOfPoints - step)/numberOfPoints)
            const y: number = (this.y1 * step/numberOfPoints) + (this.y2 * (numberOfPoints - step)/numberOfPoints)
...
export class Day05 extends Day {
    ...
    protected solvePartOne(input: string): number {
        const lines: Line[] = this.splitLines(input).map(line => new Line(line));
