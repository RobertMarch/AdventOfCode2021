...
export class Day03 extends Day {
    ...
    public solvePartOne(input: string): number {
        ...
        let gamma_rate: string = oneCountsMap.map(count => count > lines.length/2 ? '1' : '0').join('');
        let epsilon_rate: string = oneCountsMap.map(count => count < lines.length/2 ? '1' : '0').join('');
