...
export class Day07 extends Day {
    ...
    protected solvePartOne(input: string): number {
        const crabs: number[] = input.split(',').map(val => parseInt(val)).sort((a, b) => a - b);
        const optimalPosition: number = crabs[Math.floor((crabs.length - 1) / 2)];
        return crabs.reduce((total: number, currPosition: number) => total + Math.abs(currPosition - optimalPosition), 0);
    }
