...
export class Base3DCoordinate {
  public static fromArray(coordinates: number[]): Base3DCoordinate {
    if (coordinates.length !== 3) {
      throw new Error(
        `Unexpected number of coordinates given: expected 3, got ${coordinates.length}`
      );
    }
  ...
  public getVectorFromOtherCoordinate(
    other: Base3DCoordinate
  ): Base3DCoordinate {
    return new Base3DCoordinate(
      this.x - other.x,
      this.y - other.y,
      this.z - other.z
    );
